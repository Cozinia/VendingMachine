...
void setup() {
 ...
 //  sqlite3 *db2;
  ...
  //CREATE DB
   rc = db_exec(db1, "CREATE TABLE Users (ID INTEGER  PRIMARY KEY, FirstName VARCHAR2, LastName VARCHAR2,username VARCHAR2, Password VARCHAR2 NOT NULL,Credit   FLOAT(4,2)  );");
   if (rc != SQLITE_OK) {
       sqlite3_close(db1);
     //  sqlite3_close(db2);
       return;
   ...
   //INSERT VALUES IN DB
   rc = db_exec(db1, "INSERT INTO Users VALUES (1, 'Cozinia', 'Motronu', 'cozinia.motronu', '1234', 7.50);");
   if (rc != SQLITE_OK) {
       sqlite3_close(db1);
       return;
   }
